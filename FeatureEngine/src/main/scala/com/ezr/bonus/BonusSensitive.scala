...
object BonusSensitive {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("BonusSensitive" + shardingGrpId).config("spark.serializer", "org.apache.spark.serializer.KryoSerializer").enableHiveSupport().getOrCreate()
    getBonus(spark, shardingGrpId)
    spark.stop()
  }
  ...
  /**
    ...
    * @param shardingGrpId
    */
  def getBonus(spark: SparkSession, shardingGrpId: String) = {
    ...
    Utils.hiveSets(spark, ConfigHelper.env + shardingGrpId, "BonusSensitive" + shardingGrpId)
    Utils.toBehavior(spark, rowRDD, table)

    val perfRDD = spark.sql(s" select vipid, brandid, copid, count from $behavior3  where acttype = 9").rdd.map(v=>{
      val vipid = v.getAs[Long]("vipid")
      val brandid = v.getAs[Int]("brandid")
      val copid = v.getAs[Int]("copid")
    ...
    val writeDF = spark.createDataFrame(perfRDD, schema)
